...
// Vérification et création des fichiers de log si nécessaire
$logFiles = ['bots_log.txt', 'visits.log', 'vpn_proxy.log'];
...
class HMBSecurityTracker {
    ...
    private $botsSignatures = [
        ...
        'scrapy', 'lighthouse', 'wkhtmltopdf', 'phantomjs', 'zombie',
        'slurp', 'facebook', 'twitter', 'slack', 'telegram',
    ...
    private function handleBot() {
        ...
        // Log du bot
        $logEntry = sprintf(
            "[%s] BOT DETECTED | IP: %s | User-Agent: %s | Referer: %s\n",
            date('Y-m-d H:i:s'),
            $ip,
            $userAgent,
    ...
    private function sendBotNotification($ip, $userAgent) {
        try {
            ...
            $mail->Subject = "🤖 Bot Détecté sur " . $referer;
        ...
        <body>
            <div class='container'>
                ...
                <div class='content'>
                    ...
                    <p><strong>Referer:</strong> " . ($_SERVER['HTTP_REFERER'] ?? 'Direct') . "</p>
                </div>
            </div>
        </body>
        </html>";
    ...
    private function handleVPN() {
        ...
        // Log de la tentative VPN/Proxy
        $logEntry = sprintf(
            "[%s] VPN/PROXY BLOCKED | IP: %s | User-Agent: %s | Referer: %s\n",
            date('Y-m-d H:i:s'),
            $ip,
            $userAgent,
            $_SERVER['HTTP_REFERER'] ?? 'Direct'
        ...
        // Nettoyage du buffer et envoi de la page de blocage
        ob_clean();
    ...
    private function sendVPNNotification($ip, $userAgent) {
        try {
            ...
            $referer = isset($_SERVER['HTTP_REFERER']) ? parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST) : 'Accès Direct';
            $mail->Subject = "⚠️ Tentative d'accès VPN/Proxy bloquée sur " . $referer;
